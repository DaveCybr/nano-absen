...
import { Plus, Edit2, FileText, Download, RefreshCw, CheckCircle, AlertCircle, X } from 'lucide-react'
...
interface BalanceRow extends LeaveBalance {
  employee: { id: string; full_name: string; employee_code: string; group: { name: string } | null }
...
export default function LeavePage() {
  ...
  const resolveEmpIds = async (group: string): Promise<string[] | null> => {
    ...
    const { data } = await supabase.from('employees').select('id').eq('group_id', group).eq('is_active', true)
  ...
  const fetchBalances = useCallback(async () => {
    ...
    try {
      ...
      if (empIds !== null) {
        if (empIds.length === 0) { setBalances([]); setTotalBal(0); setLoadingBal(false); return }
  ...
  const handleDownloadBalance = async () => {
    ...
    try {
      ...
      exportCsv(`leave-balance_${yearFilter}`, [
        'Kode', 'Nama', 'Grup', 'Jenis Cuti', 'Limit/Tahun', 'Terpakai (Tahunan)', 'Terpakai (Lainnya)', 'Sisa',
      ], (data || []).map((b: any) => {
  ...
  return (
    <div>
                        ...
                        <tr key={b.id} className="hover:bg-gray-50/60">
                          ...
                          <td className="text-center">
                            {sisa === null ? (
                              ...
                              <span className={`font-semibold text-sm ${sisa === 0 ? 'text-red-500' : sisa <= 3 ? 'text-orange-500' : 'text-green-600'}`}>
                                {sisa}
                              </span>
                            )}
                          </td>
                          <td>
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className="btn-icon text-blue-500" title="Detail">
                                <FileText size={14} />
                              </button>
                              <button
                                onClick={() => openEditBal(b)}
                                className="btn-icon text-gray-400" title="Edit">
                                <Edit2 size={14} />
                              </button>
                            </div>
                          </td>
                      ...
                      <tr key={cat.id} className="hover:bg-gray-50/60">
                        ...
                        <td>
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => setDeleteCat(cat)} className="btn-icon text-red-400"><X size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))
      ...
      <Modal open={!!detailBal} onClose={() => setDetailBal(null)} title="Detail Saldo Cuti" width="max-w-sm">
        {detailBal && (() => {
          const limit = detailBal.leave_category?.limit_per_year
          const taken = (detailBal.annual_taken ?? 0) + (detailBal.other_taken ?? 0)
          ...
          return (
            <div className="space-y-3 text-sm">
              {[
                ...
                { label: 'Grup',          value: detailBal.employee?.group?.name || '-' },
                { label: 'Jenis Cuti',    value: detailBal.leave_category?.leave_name },
                { label: 'Tahun',         value: detailBal.year },
                { label: 'Limit / Tahun', value: limit != null ? `${limit} hari` : 'Tidak terbatas' },
                { label: 'Terpakai (Tahunan)', value: `${detailBal.annual_taken ?? 0} hari` },
                { label: 'Terpakai (Lainnya)', value: `${detailBal.other_taken ?? 0} hari` },
              ...
              {limit != null && (
                <div className="pt-1">
                  <div className="flex justify-between text-xs text-gray-400 mb-1">
                    <span>Terpakai</span><span>{taken}/{limit}</span>
                  </div>
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${Math.min(100, (taken / limit) * 100)}%` }}
                    />
                  </div>
                </div>
              )}
        ...
        <div className="flex justify-end mt-5 pt-4 border-t border-gray-100">
          <button onClick={() => setDetailBal(null)} className="btn-secondary">Tutup</button>
        </div>
      </Modal>

      {/* ── Edit Balance Modal ── */}
      <Modal open={!!editBal} onClose={() => setEditBal(null)} title="Edit Saldo Cuti" width="max-w-sm">
        {editBal && (
          <div className="space-y-4">
            <div className="p-3 bg-gray-50 rounded-xl text-sm">
              <p className="font-medium text-gray-900">{editBal.employee?.full_name}</p>
              <p className="text-gray-500 text-xs">{editBal.leave_category?.leave_name} · {editBal.year}</p>
            </div>
            {editBalError && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">{editBalError}</div>
            )}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="form-label">Annual Taken</label>
                <input type="number" min={0} className="form-input" value={editAnnual}
                  onChange={e => setEditAnnual(e.target.value)} />
              </div>
              <div>
                <label className="form-label">Other Taken</label>
                <input type="number" min={0} className="form-input" value={editOther}
                  onChange={e => setEditOther(e.target.value)} />
              </div>
            </div>
            <p className="text-xs text-gray-400">
              Limit: {editBal.leave_category?.limit_per_year != null ? `${editBal.leave_category.limit_per_year} hari` : 'Tidak terbatas'}
            </p>
          </div>
        )}
        <div className="flex gap-2 justify-end mt-5 pt-4 border-t border-gray-100">
          <button onClick={() => setEditBal(null)} className="btn-secondary">Batal</button>
          <button onClick={handleSaveBal} disabled={savingBal} className="btn-primary">
