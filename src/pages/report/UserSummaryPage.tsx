...
export default function UserSummaryPage() {
    ...
    try {
      ...
      const results: SummaryRow[] = employees.map((emp: any) => {
        ...
        return {
          ...
          in_location: d.filter((r) => r.location_in_status === "in_area")
            .length,
          tolerance_location: d.filter(
            (r) => r.location_in_status === "tolerance",
          ).length,
          out_location: d.filter((r) => r.location_in_status === "out_of_area")
            .length,
          correction_location: d.filter(
  ...
  const numCell = (val: number, color = "text-gray-700") => (
    <td
      className={`text-center font-medium ${val > 0 ? color : "text-gray-400"}`}
    >
      {val}
    </td>
  );
